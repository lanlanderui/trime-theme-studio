...
- 内置轻量配色方案管理器，可列出所选 YAML 中的全部方案，并批量修改名称或删除方案，最后统一保存和部署。
...
选择主题文件后，也可以点击“管理已有配色方案”查看文件中的全部方案。修改名称只会更新方案的 `name`，不会改变固定 ID 或浅色／深色切换关系；App 也会阻止删除最后一个方案。改名和删除会先暂存在管理器中，全部调整完成后点击“保存并部署”，App 才会统一备份、写入文件并只请求一次同文重新部署。取消管理则不会修改文件，保存后仍可使用“撤销上次写入”恢复。
